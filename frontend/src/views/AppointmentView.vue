    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-2 text-sm text-forest/80">Gestiona las citas del bebé activo o selecciona un perfil en la sección de bebés.</p>
        </div>
        <RouterLink v-if="!babyId" to="/bebes" class="btn-primary">Ir a bebés</RouterLink>
      </div>

      <div v-if="!babyId" class="rounded-4xl border border-sage bg-white/90 p-8 text-center text-forest/80">
        <p class="text-lg font-semibold">No hay bebé activo</p>
        <p class="mt-3">Selecciona un bebé en la pantalla de gestión para empezar a administrar sus citas.</p>
        <RouterLink to="/bebes" class="btn-primary mt-6 inline-flex">Ver bebés</RouterLink>
      </div>
      ...
      <div v-else>
        ...
        <div v-else-if="babiesStore.error || appointmentsStore.error" class="rounded-3xl border border-red-200 bg-red-50 p-6 text-red-700">
        ...
        <div v-else class="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
          <section class="card rounded-4xl p-6">
            ...
            <p class="mt-1 text-sm text-forest/75">{{ baby ? `Nacido el ${formatDate(baby.birth_date)}` : '' }}</p>
          ...
          <section class="card rounded-4xl p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button type="button" class="btn-primary" @click="openAppointmentModal()">Agregar cita</button>
            </div>

            <p class="mt-4 text-sm text-forest/75">Las citas estarán vinculadas al perfil seleccionado.</p>
          </section>
        </div>
        ...
        <section class="mt-8 rounded-4xl border border-sage bg-white/90 p-6">
          ...
          <div v-if="!appointmentsStore.appointments.length" class="mt-6 rounded-3xl border border-sage bg-mist p-8 text-center text-forest/80">
      ...
      <div class="mx-auto max-w-2xl rounded-3xl bg-white p-6 shadow-2xl">
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 class="mt-2 text-2xl font-semibold text-ink">{{ editingAppointment ? appointmentForm.title : 'Agregar nueva cita' }}</h2>
          </div>
        ...
        <form class="mt-6 space-y-4" @submit.prevent="saveAppointment">
          ...
          <div>
            ...
            <input v-model="appointmentForm.location" class="input" placeholder="Consultorio, clínica o dirección" />
          </div>
          <div>
...
const babyId = computed(() => route.query.babyId || authStore.user?.active_baby_id);
...
async function saveAppointment() {
  ...
  try {
    ...
    if (editingAppointment.value) {
      await appointmentsStore.updateAppointment(babyId.value, editingAppointment.value.id, payload);
      toast.success("Cita actualizada", "Los cambios de la cita se guardaron correctamente.");
    } else {
      await appointmentsStore.createAppointment(babyId.value, payload);
...
async function confirmDeleteAppointment(appointment) {
  ...
  try {
    await appointmentsStore.deleteAppointment(babyId.value, appointment.id);
...
if (babyId.value) {
  babiesStore.fetchBaby(babyId.value);
